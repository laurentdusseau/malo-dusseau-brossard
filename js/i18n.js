/**
 * FR / EN — IP France → FR, sinon EN. Préférence persistée en localStorage (pas de ?lang= en URL).
 */
(function () {
  ...
  const dict = {
    fr: {
      ...
      "dp.aud.ig": "followers",
      "dp.aud.ig.meta": "1,1 M vues en 90 j sur 2026",
      ...
      "dp.need.title": "Le coût de l'excellence",
      "dp.need.lead":
      ...
      "dp.pack.title": "Packs partenaires",
    ...
    en: {
      ...
      "dp.aud.ig": "followers",
      "dp.aud.ig.meta": "1.1M views in 90 days in 2026",
      ...
      "dp.need.title": "The cost of excellence",
      "dp.need.lead":
  ...
  function init() {
    ...
    if (chosen) {
      applyLang(chosen, { persist: true });
      return;
    }

    applyLang(fallbackLang(), { persist: false });
    fetchCountry().then((cc) => {
      if (savedLang()) return;
      if (!cc) return;
      const next = FR_COUNTRIES[cc] ? "fr" : "en";
      if (next !== window.__maloLang) {
        applyLang(next, { persist: false });
      }
    });
  }
